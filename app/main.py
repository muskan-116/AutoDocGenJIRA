...
MONGODB_URI = os.getenv("MONGODB_URI")        # ✅ main.py uses MONGODB_URI
...
from app.routers.jira_routes import router as jira_router
...
# ------------------ Services ------------------
from app.services.trello_service import connect_to_trello, register_trello_webhook
...
async def startup():
    ...
    # ✅ Single MongoDB client for entire app
    ...
    # ✅ All indexes in one place — no duplicate connections
    await db["notifications"].create_index(
        "action_id", unique=True, sparse=True
    )
    print("✅ notifications index ensured")

    # ✅ Jira oauth_states TTL index — auto-deletes expired states after 5 min
    await db["oauth_states"].create_index(
        "expires_at",
        expireAfterSeconds=0,
        name="oauth_states_ttl"
    )
    print("✅ oauth_states TTL index ensured")

    # ✅ Jira tokens index — fast user lookup
    await db["jira_tokens"].create_index(
        "user_id",
        unique=True,
        name="jira_tokens_user_id"
    )
    print("✅ jira_tokens index ensured")
        ...
        for user in users:
            ...
            try:
                res = await client_http.get(
                    ...
                    params={"key": TRELLO_API_KEY, "token": token, "fields": "id,name"}
                )
            ...
            for board in boards:
                ...
                try:
                    ...
                    if any(
                        hook.get("idModel") == board_id and
                        hook.get("callbackURL") == TRELLO_CALLBACK_URL
                    ...
                    print(f"❌ Webhook failed for board {board_id}: {e.response.status_code}, {e.response.text}")
                except Exception as e:
                    print(f"❌ Unexpected error for board {board_id}: {e}")
